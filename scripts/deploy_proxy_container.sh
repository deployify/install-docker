...
sudo docker run --name $CONTAINER --network deployify --user $USER_GROUP --restart=always -p80:80 -p81:81 -p443:443 -d -v $DATA/config:/config -v $DATA/certs:/certs $IMAGE
